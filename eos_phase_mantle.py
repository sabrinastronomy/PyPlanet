...
class AdiabaticMantleEOS:
    def __init__(self, S_setting = 3000, P_mantle_planet = np.logspace(1, 11, 100), need_plot=False):
        ...
        self.mantle_global_mat = np.asarray(["" for x in range(len(self.mantle_global_Cp))])
    ...
    def interpolate_everything(self):
        ...
        material_types = ["molten", "mix_mol_en", "mix_pv_en", "en", "pv", "ppv"]
        i = 0
    ...
    def test_plot(self):
        plt.scatter(self.mantle_global_P, self.mantle_global_T, c=self.mantle_global_Cp)
        plt.vlines(23e9, 1000, 6000, color="k")
        plt.text(25e9, 4000, "perovskite")
        plt.text(23e7, 4000, "enstatite")

        plt.colorbar(label=r"$\rho$")
        plt.xlabel("P [Pa]")
        plt.ylabel("T [K]")
        plt.title("Test Planet")
...
if __name__ == "__main__":  # only executes if running run.py versus calling a function
    new_mantle = AdiabaticMantleEOS(need_plot=True)
