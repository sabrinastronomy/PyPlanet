...
class EoS:
    ...
    def mantle_constant_eos_function_generate(self):
        ...
        self.pressures_other = np.linspace(100, p_cmb, int(1e4))  # pressures in upper mantle layers
        ...
        if p_cmb == 0: ### edge case, NO MANTLE, ONLY CORE ###
        ...
        for mm, layer in zip(self.layer_mm, self.layers):
            if type(mm) != list:  # case for one phase mantle, core, etc.
                ...
                if layer.name_layer == "layer1":  # mantle
                    ...
                    self.layer_eos.append([create_function(pressures, layer_rho_data),
                                           ...
                                           self.get_temp[
                                               -1]])  # all needed interpolations from pressure to density,
                ...
                if anchor_temperature < 2500:  # case when olivine in upper mantle and iron perovskite in lower mantle
                    self.pressures = []
                    self.temperatures = []
                    for i, lay in enumerate(layer.composite):
                        if pressures_holder[i] == "":
                            ...
                            layer_cp_data = \
                            lay.evaluate(['molar_heat_capacity_v'], pressures_holder[1], temperatures_holder[1])[0] / \
                            mm[1]
                            self.pressures = pressures_holder[1]
                            self.temperatures = temperatures_holder[1]
                            break
                        layer_rho_data.extend(
                            (lay.evaluate(['density'], pressures_holder[i], temperatures_holder[i])[0]))
                        layer_cp_data.extend(
                            (lay.evaluate(['molar_heat_capacity_v'], pressures_holder[i], temperatures_holder[i])[0]) /
                            mm[i])
                    ...
                    layer_rho_data.append(
                        (layer[0].evaluate(['density'], pressures_holder[0], temperatures_holder[0])[0]))
                    layer_cp_data.append(
                        (layer[0].evaluate(['molar_heat_capacity_v'], pressures_holder[0], temperatures_holder[0])[0]) /
                        mm[i])
                    ...
                    layer_rho_data = np.concatenate((layer_rho_data[0], layer_rho_data[
                        1]))  # makes this one dimensional array through concatenation
    ...
    def mantle_adiabatic_eos_function_generate(self):
        ...
        #     olivine_trans_max_pressuupper_lower_mantle(max(adiabatic_temperatures_mantle))
        ...
        for mm, layer in zip(self.layer_mm, self.layers):
            if type(mm) != list:  # case for one phase mantle, core, etc.
                ...
                self.temperatures.append(
                    adiabatic_temperatures)  # first pressure is 0, anchor_temperature is surface temperature
                self.get_temp.append(create_function(pressures, self.temperatures))
                if layer.name_layer == "layer1":  # mantle
                    layer_rho_data = layer.composite.evaluate(['density'], pressures, adiabatic_temperatures)[0]
                    layer_cp_data = \
                    layer.composite.evaluate(['molar_heat_capacity_v'], pressures, adiabatic_temperatures)[0] / mm
                    self.layer_eos.append([create_function(pressures, layer_rho_data),
                                           create_function(pressures, layer_cp_data),
                                           self.get_temp[
                                               -1]])  # all needed interpolations from pressure to density, heat capacity, and temperature
            else:
                """
                case when there is both an upper and lower mantle
                """
                ...
                temperatures_upper = geotherm.adiabatic(pressures_upper, anchor_temperature,
                                                        layer.composite[
                                                            1])  # we have to do this in reverse so using enstatite (1)
        ...
        if p_c == p_cmb:
            ...
            if self.temp_profile == "_adiabatic_":
                if len(self.temperatures) > 1:  # there is a mantle
                    first_layer_temps = self.temperatures[
                        -1]  # lower mantle top temperature which is anchor temperature
                else:
