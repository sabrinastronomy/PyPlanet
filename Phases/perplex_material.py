...
def create_perplex_table(werami_path, project_name, outfile, n_pressures, n_temperatures, pressure_range=None,
...
class PerplexMaterial(Material):
    ...
    def _read_2D_perplex_file(self, filename):
        ...
        property_interpolators = {'rho': interp2d(pressures, temperatures, densities.T),
                                  'alpha': interp2d(pressures, temperatures,
                                                    property_table[:, :, p_indices[1]][:, :, 0].T),
                                  'K_T': interp2d(pressures, temperatures,
                                                  1.e5 / property_table[:, :, p_indices[2]][:, :, 0].T),
                                  'K_S': interp2d(pressures, temperatures,
                                                  1.e5 * property_table[:, :, p_indices[3]][:, :, 0].T),
                                  'G_S': interp2d(pressures, temperatures,
                                                  1.e5 * property_table[:, :, p_indices[4]][:, :, 0].T),
                                  'bulk_sound_velocity': interp2d(pressures, temperatures,
                                                                  property_table[:, :, p_indices[5]][:, :, 0].T),
                                  'p_wave_velocity': interp2d(pressures, temperatures,
                                                              property_table[:, :, p_indices[6]][:, :, 0].T),
                                  's_wave_velocity': interp2d(pressures, temperatures,
                                                              property_table[:, :, p_indices[7]][:, :, 0].T),
                                  'S': interp2d(pressures, temperatures,
                                                property_table[:, :, p_indices[8]][:, :, 0].T * molar_masses.T),
                                  'H': interp2d(pressures, temperatures,
                                                property_table[:, :, p_indices[9]][:, :, 0].T * molar_masses.T),
                                  'C_p': interp2d(pressures, temperatures,
                                                  property_table[:, :, p_indices[10]][:, :, 0].T * molar_masses.T),
